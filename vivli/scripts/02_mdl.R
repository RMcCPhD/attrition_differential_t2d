...
# Replace zeros with NAs where variables were not fitted
# Matrices captured all variables even if not fitted, retaining for second
# stage in case full structure is needed for multinma
# Additional stage to remove NA rows and columns, keep square matrix
d_vcov <- c_gather_df %>% 
  select(nct_id, modeltype, vcov) %>% 
  mutate(vcov = map(vcov, ~ cov2cor(.x)))

# Tidy vcov
# Remove NAs
d_tidy_vcov <- d_vcov %>% 
  mutate(
    vcov_long = map2(nct_id, vcov, ~ {
      mat <- .y
      melted <- melt(mat, varnames = c("v1", "v2"), value.name = "cor")
      as_tibble(melted)
    })
  ) %>% 
